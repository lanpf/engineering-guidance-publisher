...
- **强制 · 主题**：必须依据项目权威职责与领域文档、逻辑所表达的业务语义判定归属，不得仅按类名、包名、注解或条件分支数量判定分层。协议解析、序列化、连接管理及不改变业务语义的技术重试属于适配行为；Gateway 的协议认证和路由执行也须按实际语义区分技术执行与业务决策，不得一概迁入 domain/application。
- **强制 · 主题**：检查领域职责时必须定位资格判定、业务不变量、状态转换及业务决策的实际执行位置；同一规则应内聚于负责该语义的领域模型或领域服务，由各入口复用，不得在 controller、filter、listener、adapter 或 application 中各自维护业务判定。协议格式校验与领域有效性校验必须区分，具体校验边界遵循[校验与异常最佳实践](bp_validation.md)。
- **强制 · 主题**：检查应用职责时必须沿用例调用链定位业务步骤顺序、多个领域能力或端口的协调、业务结果分支及业务补偿决策；这些编排必须归属 application。适配器内部为完成一次技术操作而进行的 SDK 调用、分页拉取或技术重试不因包含多个步骤就视为应用编排；独立的基础设施恢复流程遵循[业务补偿最佳实践](bp_compensation.md)。
- **强制 · 主题**：不得以已存在 domain/application module 或已通过中间 service 转发作为职责内聚的证据；必须追踪到实际作出业务决策及编排的实现。迁移职责时必须迁移相应决策和行为验证，不能仅移动类、改包名或增加转发层而保留外层原有业务逻辑。
- **强制 · 主题**：依赖方向检查必须同时覆盖 POM 中实际声明或继承的依赖与源码中的 import、全限定类型引用、方法签名、继承/实现关系及注入类型；沿相关调用链检查隐式反向依赖。必须按本文“依赖方向”和“层内约定”的允许范围判定，不能仅凭编译成功、没有循环依赖或接口抽象就认定分层正确。
