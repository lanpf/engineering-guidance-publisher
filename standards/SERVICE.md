...
- 每个服务必须在工程根目录维护 `DOMAIN.md`，并在文首声明所采用的规约版本，以及该版本 `COMMON.md`、`SERVICE.md` 的本地约定路径；任一规约不可读时不得构建。
- `DOMAIN.md` 只定义当前服务的领域边界、领域语言、业务规则、错误定义、领域事件和 API 语义，不复制通用工程规约，也不得放宽或覆盖通用、服务级规约；发生冲突时以上层规约为准。
