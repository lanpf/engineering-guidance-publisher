...
- **强制 · 基础**：只有 API payload 在共用返回类型时可以使用 `*ApiResponse`，只有 application 在共用返回类型时可以使用 `*Response`；外部服务调用的本地协议模型使用“外部服务名 + 动作或资源 + `RequestPayload`/`ResponsePayload`”命名，例如 `GithubSearchRepositoriesRequestPayload`、`GithubSearchRepositoriesResponsePayload`，不得使用缺少外部服务身份或调用语义的 `RequestPayload`、`CreateRequestPayload` 等名称；内部服务调用无法复用被调用方 API 而定义的本地请求类型使用 `*InternalRequest`。
