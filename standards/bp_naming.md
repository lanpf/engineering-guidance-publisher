...
- 资源名称（包括但不限于缓存键、分布式锁键）的生效范围由命名空间限定：最终名称由命名空间前缀与业务键组成，命名空间必须能区分部署环境与应用，防止跨环境、跨应用串扰。
- 业务键只在命名空间内要求唯一；同一业务键在不同环境或应用中解析为不同的物理资源。
- 命名空间只表达部署身份，不得承载业务语义；业务语义只出现在业务键部分。
